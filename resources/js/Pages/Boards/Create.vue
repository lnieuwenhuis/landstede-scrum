...
const weekdays = [__('Monday'), __('Tuesday'), __('Wednesday'), __('Thursday'), __('Friday'), __('Saturday'), __('Sunday')];
const selectedWeekdays = ref([__('Saturday'), __('Sunday')]);
...
const generateSprints = () => {
    if (!dateRange.value) {
        toast.warning(__('Please select start and end dates first'));
        return;
    }
...
const submitForm = async () => {
    if (!board.value.title || !board.value.description || !board.value.startDate || !board.value.endDate) {
        toast.warning(__('Please fill in all required fields'));
        return;
    }
    ...
    try {
        ...
        if (response.data.status === 'redirect') {
            ...
            toast.success(__('Board created successfully'));
        }
    } catch (error) {
        console.error('Error creating board:', error);
        if (error.response && error.response.data && error.response.data.errors) {
            console.error("Validation Errors:", error.response.data.errors);
            toast.error(error.response.data.message || __('Validation failed. Please check your input.'));
        } else {
            toast.error(__('Failed to create board. Please try again.'));
        }
    } finally {
                    ...
                    <div class="p-6">
                        <form @submit.prevent="submitForm">
                            ...
                            <div class="mb-6">
                                ...
                                <div class="space-y-4">
                                    <div>
                                        <label for="title" class="block text-sm font-medium text-gray-700">{{ __('Board Title') }}</label>
                                        <input 
                                            type="text" 
                                            id="title" 
                                            v-model="board.title" 
                                    ...
                                    <div>
                                        ...
                                        <textarea 
                                            ...
                                            rows="3"
                                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                                            :placeholder="__('Enter board description')"
                                            required
                                        ></textarea>
                                    </div>
                                </div>
                            ...
                            <!-- Date Range -->
                            <div class="mb-6">
                                <h3 class="text-lg font-semibold text-gray-900 mb-4">{{ __('Project Timeline') }}</h3>
                                ...
                                <div v-if="dateRange" class="mt-2 text-sm text-gray-500">
                                    {{ __('Project duration:') }} {{ totalDays }} {{ __('days') }}
                                </div>
                            </div>
                            ...
                            <div class="mb-6">
                                <div class="flex justify-between items-center mb-4">
                                    <h3 class="text-lg font-semibold text-gray-900">{{ __('Sprints') }}</h3>
                                    <button 
                                        type="button"
                                ...
                                <div v-if="!showSprintEditor && !board.sprints.length" class="text-center py-4 text-gray-500 border border-dashed border-gray-300 rounded-lg">
                                    {{ __('No sprints created yet. Click "Generate Sprints" to create them automatically.') }}
                                </div>
                            ...
                            <!-- Non-working Days -->
                            <div class="mb-6">
                                <h3 class="text-lg font-semibold text-gray-900 mb-4">{{ __('Non-working Days') }}</h3>
                                
                                <!-- Weekday Selection -->
                                <div class="mb-4">
                                    <label class="block text-sm font-medium text-gray-700 mb-2">{{ __('Select Non-working Weekdays') }}</label>
                            ...
                            <div class="flex justify-end">
                                ...
                                >
                                    <span v-if="isSubmitting">{{ __('Creating...') }}</span>
                                    <span v-else>{{ __('Create Board') }}</span>
                                </button>
                            </div>
                        </form>
                    </div>
